...
- **端云协同成本控制框架**
  ...
  - 为了让成本控制可视化，我实现了一个实时成本面板，按 `Ctrl+Shift+D` 可以调出。面板指标由 BFF 的 `CostService` 通过 WebSocket `metrics:result` 事件推送，前端只负责展示；指标包括视觉/LLM 调用次数、输入/输出 Tokens、预估费用、帧捕获/跳过、缓存命中、平均响应时间和当前 RPM。
...
- **对话状态机调度器（Orchestrator）**
  ...
  - 比如用户按住空格时进入 `listening`，松开时停止 ASR 并把最终结果送进下一状态；ASR 识别完成后**强制捕获一帧当前画面**（不依赖前端变化检测），然后和文字一起发给 BFF；BFF 返回结果后进入 `speaking` 调用 TTS；按 ESC 可以强制中断当前轮次。这个状态机帮我理清了多模态之间的时序关系。
...
2. **帧间变化检测（ChangeDetector + PixelDiffDetector）**
   ...
   - 这个机制主要用于**持续推视频流**（`frame` 消息）场景；而在**用户主动按住麦克风发起的对话**中，会强制捕获当前画面并发送给 BFF，保证 AI 看到的是用户提问时的实时画面，避免前端误过滤。
...
- **前端变化检测**：`PixelDiffDetector` 对相邻帧做像素级差异采样。如果画面变化很小（比如用户只是轻微晃动），就直接丢弃，不发到后端。注意：用户主动发起的对话会强制发帧，不走这层过滤。
- **后端感知哈希去重**：BFF 收到帧后计算 64 位感知哈希，和该用户上一帧比对。哈希相同说明画面几乎没变，直接跳过视觉 API。
- **对话流程缓存复用**：`dialogue` 消息中携带的画面也会计算感知哈希并查询 LRU 缓存，相同画面连续提问时只有第一次真正调用视觉模型。
- 这几层去重叠加起来，对于静态或慢变场景能过滤掉绝大部分冗余调用。
...
- 该缓存同时覆盖 `frame` 实时帧和 `dialogue` 对话帧两条链路。
